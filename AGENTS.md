...
After editing Python files (like `DP_APOD/routes.py`):

```bash
python -m py_compile DP_APOD/__init__.py  # Check single file for syntax errors
```
...
- Backend processing in `DP_APOD/routes.py`
...
- **✅ Do**: `DP_APOD/` with underscores, all lowercase
...
```typescript
const PLUGIN_ID = 'DP_APOD:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'DP_APOD:open-panel';
  export const refreshData = 'DP_APOD:refresh-data';
}
...
- Backend routes: `DP_APOD/routes.py` (class `DataRouteHandler`)
- Frontend logic: `src/` directory
- Python package: `DP_APOD/` directory
...
1. **Read the backend first** — Check `DP_APOD/routes.py` to understand the existing API contract
...
Create RESTful endpoints in `DP_APOD/routes.py`:
...
def setup_route_handlers(web_app):
    ...
    data_route = url_path_join(base_url, "DP_APOD", "data")
...
- Define matching endpoint path strings (e.g., `"hello"`, `"get-data"`) in both `DP_APOD/routes.py` and `src/api.ts` to ensure routes sync between backend and frontend
...
2. **Write the code** (TypeScript in `src/`, styles in `style/`, Python in `DP_APOD/`)
...
5. **Install the extension** (REQUIRED for JupyterLab to recognize it):
   ...
   jupyter server extension enable DP_APOD
   ```
...
- **Output**: Creates files in `lib/` and `DP_APOD/labextension/`
...
**After editing Python** (files in `DP_APOD/`):
...
- Ask user to search for the extension ID (`DP_APOD`) to see if it loaded
...
ls -la DP_APOD/labextension/  # Should contain bundled extension
```
...
- Backend routes: `DP_APOD/routes.py`
- Server extension config: `jupyter-config/server-config/DP_APOD.json`
...
**❌ Don't**: Write frontend interfaces without first checking the backend API contract in `DP_APOD/routes.py`
...
**❌ Don't**: Use absolute imports like `from DP_APOD.routes import ...`
...
- **Plugin ID** (in `src/index.ts`): `'DP_APOD:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'DP_APOD:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-DP-APOD-ClassName`
- **API routes** (in `DP_APOD/routes.py`): `url_path_join(base_url, "DP_APOD", "endpoint")`
